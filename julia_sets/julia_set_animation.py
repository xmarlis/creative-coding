...
def create_julia_animation(num_frames=120, width=800, height=800):
    ...
    # Generate and show first frame
    angle = 0
    c_real = radius * np.cos(angle)
    c_imag = radius * np.sin(angle)
    data = generate_julia_frame(width, height, c_real, c_imag)
    
    fig_preview, ax_preview = plt.subplots(figsize=(10, 10), dpi=80, facecolor='black')
    ax_preview.axis('off')
    ax_preview.set_facecolor('black')
    cmap = plt.cm.twilight_shifted
    ax_preview.imshow(data, cmap=cmap, interpolation='bilinear', extent=[-2, 2, -2, 2])
    plt.tight_layout(pad=0)
    print(f"Displaying first frame with c = {c_real:.4f} + {c_imag:.4f}i")
    print("Close the preview window to continue generating animation...")
    plt.show()
